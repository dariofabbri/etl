...
public class TAdvancedHashEndJava
{
  ...
  protected final String TEXT_14 = "\") == null) {" + NL + "\t\t\t   \t\t\tThread.sleep(1);" + NL + "\t\t\t   \t\t}" + NL + "\t\t\t   \t" + NL + "\t\t\t\t} " + NL + "\t\t\t}" + NL + "" + NL + "\t}" + NL + "" + NL + "} " + NL + "} catch (Exception ex) {" + NL + "\tglobalMap.put(\"THREADS_HAVE_ERROR\",ex);" + NL + "\tthrow ex;" + NL + "} catch (Error er) {" + NL + "\tglobalMap.put(\"THREADS_HAVE_ERROR\",er);" + NL + "\tthrow er;" + NL + "}" + NL + "" + NL + "}//end synchronized" + NL + NL;
